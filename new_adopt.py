...
def dump_data():
	global data, labels, empty_commits	#use global variables
	...
	#save empty commit data to file - if exists
	if len(empty_commits) != 0:
		pik = ("data_files/new_event_features/%s/%s_empty_events.pkl" % (data_year, data_month))
		with open(pik, "wb") as f:
			pickle.dump(empty_commits, f)

	#clear data, labels, and empty_commits
	data = []
	labels = []
	empty_commits = []
...
def process_commit(c):
	...
	global s
	global data, labels, empty_commits, data_month, data_year
	...
	commit_features.extend(user.get_user_features(time))		#add on user-only features, also consistent
	...
	for lib in added_libs:
		...
		feature_vector = list(commit_features)	#includes user features
	...
	#if no import events (from either added or updated), write commit data to list
	if len(added_libs) == 0 and len(updated_libs) == 0:
		empty_commits.append(commit_features)
