...
class User:
	...
	#return a vector of user-package features (skips the user-only features given by get_features)
	...
	#log new user commit(not necessarily a library commit) - but only track enough information to populate features of negative samples - along with purpose of commit: regular commit, lib import, or adoption
	def log_commit_minimal(self, time, repo, implicit_libs, has_lib = False, adopt = False):
		self.commit_count += 1		#update overall commit count
