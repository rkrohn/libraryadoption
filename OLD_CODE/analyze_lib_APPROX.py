#approximate (and therefore faster) version of analyze_lib.py
...
for repo in repos_list:
	...
	for commit in repo_commits:		#each commit is user, time, dictionary of imports
		...
		if "+" in commit[2] and ((SUBMODULE and lib in commit[2]["+"]) or (not SUBMODULE and any(item.startswith(lib) for item in commit[2]["+"]))):
			...
			#empty timestamp? skip it and keep a count
			if len(commit[1]) == 0:
				empty_count = empty_count + 1
			elif user not in user_earliest or int(commit[1]) < user_earliest[user]:
...
print empty_count, "commits with no timestamp"
...
plot_utils.plot_data(comp_data, "Time (UNIX)", "Number of Components", "%s: Number of Components over Time" % lib, filename = "results/%s_time_components_APPROX.png" % lib, x_max = 0, x_min = 0, log_scale = False)
